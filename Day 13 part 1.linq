...
Button A: X+44, Y+95
Button B: X+47, Y+32
Prize: X=7163, Y=9560".Split(Environment.NewLine);
	...
	{
		var finalState = simulator.SimulateWin(machine, 100);
		sum += finalState.IsWin ? finalState.TotalCost : 0;
		finalState.Dump();
	}
...
public class MachinePlaySimulator
{
	...
	{
		var cheapButton = machineInfo.Buttons.OrderBy(x => x.Cost).FirstOrDefault();
		var cheapMachine = new MachineInfo
		{
			Buttons = new [] { cheapButton }.ToList(),
			PrizeLocation = machineInfo.PrizeLocation,
		};

		var cheapState = new PlayStateInfo
		{
			MachineInfo = cheapMachine,
			ClawLocation = (0, 0),
		};
		
		var cheapStateEnd = SimulateWin(cheapState, numTurns);
		if (cheapStateEnd.IsWin)
		{
			return cheapStateEnd;
		}
		
		var expensiveButton = machineInfo.Buttons.OrderByDescending(x => x.Cost).FirstOrDefault();
		
		var numCheapToRemove = 0;
		for (;cheapStateEnd.ClawLocation.X > 0 && cheapStateEnd.ClawLocation.Y > 0; cheapStateEnd.ClawLocation = (cheapStateEnd.ClawLocation.X - cheapButton.XMovement, cheapStateEnd.ClawLocation.Y - cheapButton.YMovement), numCheapToRemove++)
		{
			if ((machineInfo.PrizeLocation.X - cheapStateEnd.ClawLocation.X) % expensiveButton.XMovement == 0 && (machineInfo.PrizeLocation.Y - cheapStateEnd.ClawLocation.Y) % expensiveButton.YMovement == 0)
			{
				break;
			}
		}
		
		var numExpensiveButtons = (machineInfo.PrizeLocation.X - cheapStateEnd.ClawLocation.X) / expensiveButton.XMovement;

		cheapStateEnd.TotalCost -= numCheapToRemove * cheapButton.Cost;
		cheapStateEnd.TotalCost += numExpensiveButtons * expensiveButton.Cost;
		cheapStateEnd.ClawLocation = (cheapStateEnd.ClawLocation.X + numExpensiveButtons * expensiveButton.XMovement, cheapStateEnd.ClawLocation.Y + numExpensiveButtons * expensiveButton.YMovement);
